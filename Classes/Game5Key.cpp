...
CCScene* Game5Key::scene() {
	CCScene *scene = CCScene::create();
	Game5Key *layer = Game5Key::create();
	scene->addChild(layer);
	return scene;
}

void Game5Key::clearZero(float dt)
{
	if (this->curCombo == 0)
	{
...
//场景及返回按钮
	...
	CCSprite* panlGameSprite = CCSprite::createWithSpriteFrameName(
			"panl_game.png");
	CCSprite* panl5KSprite = CCSprite::createWithSpriteFrameName("panl_5k.png");
	...
	CCMenuItemImage *pCloseItem = CCMenuItemImage::create("CloseNormal.png",
			"CloseSelected.png", this, menu_selector(Game5Key::menuCallback));
	...
	panl5KSprite->setAnchorPoint(CCPointMake(0.5,1));
	...
	panl5KSprite->setPosition(
			ccp(origin.x+visibleSize.width/2,origin.y+visibleSize.height-panlGameSprite->getContentSize().height+1));
	this->addChild(panl5KSprite, 2);
	this->addChild(top, 5);

	//顶点出生音符宽度；
	float bornPerx = panlGameSprite->getContentSize().width / 8 / 5;
	float tleftx = origin.x + visibleSize.width / 2 - 2.5 * bornPerx;
	float trightx = tleftx + 5 * bornPerx;
	//每个音符的初始位置
	bornNotePos1 = CCPointMake(tleftx,origin.y+visibleSize.height);
	bornNotePos2 = CCPointMake(tleftx+1.5*bornPerx,origin.y+visibleSize.height);
	bornNotePos3 = CCPointMake(tleftx+2.5*bornPerx,origin.y+visibleSize.height);
	bornNotePos4 = CCPointMake(tleftx+3.5*bornPerx,origin.y+visibleSize.height);
	bornNotePos5 = CCPointMake(trightx,origin.y+visibleSize.height);
	//内角度数,左右的边线的偏折角度
	float ndu = atan(
			(panl5KSprite->getContentSize().height / 3
					+ panlGameSprite->getContentSize().height)
					/ (panl5KSprite->getContentSize().width / 2 - 2.5 * bornPerx))
			* 180 / 3.14;
	lSide->setAnchorPoint(CCPointMake(1,0));
	lSide->setPosition(
			ccp(origin.x,origin.y+panl5KSprite->getContentSize().height/3*2));
	lSide->setRotation(90 - ndu+0.5);
	this->addChild(lSide, 4);
	rSide->setAnchorPoint(CCPointMake(0,0));
	rSide->setPosition(
			ccp(origin.x+visibleSize.width,origin.y+panl5KSprite->getContentSize().height/3*2));
	rSide->setRotation(-(90 - ndu +0.5));
	this->addChild(rSide, 4);
	//中间的间隔线的偏折
	float perns = (panlGameSprite->getContentSize().width
			- panl5KSprite->getContentSize().height / 3 * 2) / 5;//////////////底部///////////////////////////////////////////////////
	float nndu = atan(
			1.5*(perns - bornPerx) / panlGameSprite->getContentSize().height) * 180
			/ 3.14;
	float nndu1 = atan(
		(0.5*(perns - bornPerx)) / panlGameSprite->getContentSize().height) * 180
		/ 3.14;
	...
	sider_2->setPosition(
			ccp(origin.x+visibleSize.width/2-0.5*perns,origin.y+panl5KSprite->getContentSize().height));
	sider_1->setPosition(
			ccp(sider_2->getPosition().x-perns-2.5,sider_2->getPosition().y));
	sider_3->setPosition(
		ccp(sider_2->getPosition().x+perns+1.5,sider_2->getPosition().y));
	sider_4->setPosition(
		ccp(sider_2->getPosition().x+2*perns+4.5,sider_2->getPosition().y));
	sider_1->setRotation(nndu);
	sider_2->setRotation(nndu1);
	sider_3->setRotation(-nndu1);
	sider_4->setRotation(-nndu);
	...
	star1[0] =
			ccp(origin.x+visibleSize.width/2-2.5*bornPerx - perKeyW/2,origin.y+visibleSize.height);
	star1[1] =
			ccp(origin.x+visibleSize.width/2-2.5*perns,origin.y+panl5KSprite->getContentSize().height);
	star1[2] =
			ccp(origin.x+visibleSize.width/2-1.5*perns-1,origin.y+panl5KSprite->getContentSize().height);
	star1[3] =
			ccp(origin.x+visibleSize.width/2-1.5*bornPerx-1,origin.y+visibleSize.height);

	star2[0] =
			ccp(origin.x+visibleSize.width/2-1.5*bornPerx+1,origin.y+visibleSize.height);
	star2[1] =
			ccp(origin.x+visibleSize.width/2-1.5*perns+2,origin.y+panl5KSprite->getContentSize().height);
	star2[2] =
			ccp(origin.x+visibleSize.width/2-2-0.5*perns,origin.y+panl5KSprite->getContentSize().height);
	star2[3] = 
			ccp(origin.x+visibleSize.width/2-1-0.5*bornPerx,origin.y+visibleSize.height);
	...
	star3[1] =
			ccp(origin.x+visibleSize.width/2+2-0.5*perns,origin.y+panl5KSprite->getContentSize().height);
	star3[2] =
			ccp(origin.x+visibleSize.width/2+0.5*perns-2,origin.y+panl5KSprite->getContentSize().height);
	star3[3] =
			ccp(origin.x+visibleSize.width/2+0.5*bornPerx-1,origin.y+visibleSize.height);

	star4[0] =
			ccp(origin.x+visibleSize.width/2+0.5*bornPerx+1,origin.y+visibleSize.height);
	star4[1] =
			ccp(origin.x+visibleSize.width/2+0.5*perns+3,origin.y+panl5KSprite->getContentSize().height);
	star4[2] =
			ccp(origin.x+visibleSize.width/2+1.5*perns,origin.y+panl5KSprite->getContentSize().height);
	star4[3] =
			ccp(origin.x+visibleSize.width/2+1.5*bornPerx,origin.y+visibleSize.height);


	star5[0] =
		ccp(origin.x+visibleSize.width/2+1.5*bornPerx+1,origin.y+visibleSize.height);
	star5[1] =
		ccp(origin.x+visibleSize.width/2+1.5*perns+3,origin.y+panl5KSprite->getContentSize().height);
	star5[2] =
		ccp(origin.x+visibleSize.width/2+2.5*perns,origin.y+panl5KSprite->getContentSize().height);
	star5[3] =
		ccp(origin.x+visibleSize.width/2+2.5*bornPerx,origin.y+visibleSize.height);
...
//	scoreLable->setString("0");
	...
	greatRect =
			CCRectMake(panl5KSprite->getPosition().x,panl5KSprite->getPosition().y,panl5KSprite->getContentSize().width,panl5KSprite->getContentSize().height*2);
...
//背景音乐
	...
	schedule(schedule_selector(Game5Key::born), 0.5);
	schedule(schedule_selector(Game5Key::clearZero),0.5);
	schedule(schedule_selector(Game5Key::finishGame),1);
	return true;
}


void Game5Key::remove(CCNode *pSender) { //动作执行完后移除
	if (pSender == NULL) {
		return;
	}
...
int Game5Key::checkScore(CCNode *pNode) {
	int checkid = 0;
	if (pNode == NULL) {
		return checkid;
	}
...
void Game5Key::clearCurComb(CCNode *pLable) {
	if (pLable == NULL) {
		return;
	}
...
*/
void Game5Key::showCombo(int id) {
	//todo......
	if (id == 0) {
		return;
	}
	...
	CCCallFuncN* s_pCallBack = CCCallFuncN::create(this,
			callfuncN_selector(Game5Key::remove));
...
void Game5Key::born(float dt) {
	...
	it = ssmap->find("aaaa");
	if (keyValue != (it->second).key5Value.length())
	{
		string key(it->second.key5Value,keyValue,1); 
	...
	if (bornKey == 1) {
		CCSprite* musicNote_5_1 = CCSprite::createWithSpriteFrameName(
				"button_4key_3_1.png");
		musicNote_5_1->setPosition(bornNotePos1);
		musicNote_5_1->setScale(0.16f);
		this->addChild(musicNote_5_1, 4, 1);
				...
				(CCFiniteTimeAction*) CCSequence::create(
						CCSpawn::create(CCScaleTo::create(speed, 1),
								CCMoveTo::create(speed,
										ccp(origin.x+perNoteBottom_X/2-25,origin.y)),NULL),pCallback,NULL);
		musicNote_5_1->runAction(noteAction1);
	}
	if (bornKey == 2) {
		CCSprite* musicNote_5_2 = CCSprite::createWithSpriteFrameName(
				"button_4key_3_2.png");
		musicNote_5_2->setPosition(bornNotePos2);
		musicNote_5_2->setScale(0.2f);
		this->addChild(musicNote_5_2, 4, 2);
		CCFiniteTimeAction* noteAction2 =
				(CCFiniteTimeAction*) CCSequence::create(
						CCSpawn::create(CCScaleTo::create(speed, 1),
								CCMoveTo::create(speed,
										ccp(origin.x+perNoteBottom_X+perNoteBottom_X/2-10,origin.y)),NULL),pCallback,NULL);
		musicNote_5_2->runAction(noteAction2);
	}
	if (bornKey == 3) {
		CCSprite* musicNote_5_3 = CCSprite::createWithSpriteFrameName(
				"button_4key_3_3.png");
		musicNote_5_3->setPosition(bornNotePos3);
		musicNote_5_3->setScale(0.2f);
		this->addChild(musicNote_5_3, 4, 3);
		...
		musicNote_5_3->runAction(noteAction3);
	}
	if (bornKey == 4) {
		CCSprite* musicNote_5_4 = CCSprite::createWithSpriteFrameName(
				"button_4key_3_4.png");

		musicNote_5_4->setPosition(bornNotePos4);

		musicNote_5_4->setScale(0.2f);

		this->addChild(musicNote_5_4, 4, 4);
		...
		musicNote_5_4->runAction(noteAction4);
	}
	if (bornKey == 5) {
		CCLOG("5");
		CCSprite* musicNote_5_5 = CCSprite::createWithSpriteFrameName(
			"button_4key_3_4.png");

		musicNote_5_5->setPosition(bornNotePos5);

		musicNote_5_5->setScale(0.2f);

		this->addChild(musicNote_5_5, 4, 5);

		CCFiniteTimeAction* noteAction5 =
			(CCFiniteTimeAction*) CCSequence::create(
			CCSpawn::create(CCScaleTo::create(speed, 1),
			CCMoveTo::create(speed,
			ccp(origin.x+perNoteBottom_X*4+perNoteBottom_X/2+25,origin.y)), NULL),pCallback,NULL);

		musicNote_5_5->runAction(noteAction5);
	}
...
int Game5Key::containsTouchLocation(CCTouch* touch) {
...
void Game5Key::ccTouchesBegan(CCSet *touches, CCEvent *pEvent) {
	CCSetIterator iter;
	CCTouch *touch;

	for (iter = touches->begin(); iter != touches->end(); ++iter) {
		touch = (CCTouch *) (*iter);
		int touchKey = Game5Key::containsTouchLocation(touch);
		if (touchKey == 0) {
			return;
		}
		switch (touchKey) {
...
void Game5Key::zOrderSprite(CCSprite* sprite1, CCSprite* sprite2) {
...
void Game5Key::ccTouchesEnded(CCSet *touches, CCEvent *pEvent) {
	CCSetIterator iter;
	CCTouch *touch;
	for (iter = touches->begin(); iter != touches->end(); ++iter) {
		touch = (CCTouch *) (*iter);
...
void Game5Key::ccTouchesMoved(CCSet *touches, CCEvent *pEvent) {
	CCSetIterator iter;
	CCTouch *touch;
	for (iter = touches->begin(); iter != touches->end(); ++iter) {
		touch = (CCTouch *) (*iter);
...
void Game5Key::menuCallback(CCObject* pSender) {
	...
	p1->setCallbackFunc(this,callfuncN_selector(Game5Key::resumeGame));
	this->addChild(p1,15);;
}

void Game5Key::finishGame( float dt )
{
	currentSeconds++;
	if (this->lastHP > 0)
	{
...
void Game5Key::resumeGame( cocos2d::CCNode *pNode )
{
